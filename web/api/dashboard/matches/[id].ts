...
const dashboardMatchByIdHandler = async (
  request: Request
): Promise<Response> => {
  try {
    ...
    if (
      ...
      return errorJson(
        "Falta configurar SANITY_API_WRITE_TOKEN para modificar partidos.",
        500
      );
    }

    return errorJson(
      "No pudimos procesar el partido. Verifica la configuracion del backend.",
      500
    );
  }
};
...
export default {
  fetch: dashboardMatchByIdHandler,
};
