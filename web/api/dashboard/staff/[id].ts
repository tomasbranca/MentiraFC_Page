...
const dashboardStaffByIdHandler = async (
  request: Request
): Promise<Response> => {
  try {
    ...
    if (
      ...
      return errorJson(
        "Falta configurar SANITY_API_WRITE_TOKEN para modificar integrantes del plantel.",
        500
      );
    }

    return errorJson(
      "No pudimos procesar el integrante. Verifica la configuracion del backend.",
      500
    );
  }
};
...
export default {
  fetch: dashboardStaffByIdHandler,
};
